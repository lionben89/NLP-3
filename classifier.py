...
class BasicNN(Classifier):
    ...
    def train(self, X_train, y_train):
        ...
        #     with torch.no_grad():
        #         y_pred = self.model.forward(X_train)
        #         print("epoch: {}. loss:{}. metrics:{}".format(i, self.losses[-1],
        #                                                       evaluate_metrics(y_train, y_pred.max(1).indices)))
...
class TextNumericalInputsClassifier(Classifier):
    def __init__(self, input_size, n_layers, linear_dim, dense_size, numeric_feature_size, dropout=0.5, n_epochs=50,
                 batch_size=32,
                 criterion=nn.CrossEntropyLoss()):
        super().__init__()
        self.model = LSTMTextNN(input_size, n_layers, linear_dim, dense_size, numeric_feature_size, dropout=0.5)
